...
// Titulo de la tarjeta: prioridad al argumento, luego `titulo_tarjeta:` del .md,
// y si no la parte del `titulo:` antes de los dos puntos (heuristica que da un
// titular corto y limpio para todos los articulos actuales).
...
function cardTitle() {
  ...
  return t ? clean(t[1]).split(':')[0].trim() : 'SoyLegal360';
}
...
console.log(`gen-blog-image: ${Math.round(out.length / 1024)} KB -> assets/img/blog/${slug}.webp`);
console.log(`  titulo tarjeta: "${title}"`);
console.log(`  anade al frontmatter:  imagen: /assets/img/blog/${slug}.webp`);
